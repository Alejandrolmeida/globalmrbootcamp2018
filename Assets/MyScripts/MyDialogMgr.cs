...
public class MyDialogMgr : MonoBehaviour
{
    ...
    private void Update()
    {
        switch (state)
        {
            case robotState.hablando:
                ...
                if (!MyRobotMgr.instance.speechManager.audioSource.isPlaying && count > 2)
                {
                    if (activo)
                    {
            ...
            case robotState.escuchando:
                e_i.material = verde;
                e_d.material = verde;
                MyRobotMgr.instance.microphoneManager.StartCapturingAudio();
                break;
            default:
                e_i.material = normal;
                e_d.material = normal;
    ...
    public void DictationProcess(string dic)
    {
        if (activo)
        {
            ...
            MyRobotMgr.instance.speechManager.Speak(texto_out); //no existe aun
            count = 0;
